...
namespace Auditor.Extensions;

public static class PropertyEntryExtension
{
    ...
    internal static bool ValueChanged(this PropertyEntry source)
    {
        ...
        if ((source.OriginalValue == null && source.CurrentValue != null) || (source.CurrentValue == null && source.OriginalValue != null))
        {
            return true;
        }
    ...
    internal static string PrimaryKeyValue(this IEnumerable<PropertyEntry> source)
    {
        ...
        var keys = source.Where(property => property.Metadata.IsPrimaryKey() && !property.Metadata.IsForeignKey()).ToList();
        if (keys.Count == 1)
        {
            return string.Format(CultureInfo.CurrentCulture,"{0}", keys.First().CurrentValue);
        }
        return string.Join(", ", keys.Select(n => string.Format(CultureInfo.CurrentCulture, "{0}:{1}", n.Metadata.Name, n.CurrentValue)).ToList());

    }

    internal static EntityFieldAudit Convert(this PropertyEntry source, bool updated, string entityName, string entityFullName, string tableName, string primaryKey, DateTime timeStamp, string operationType, Guid eventId)
    {
        ...
        var entry = new EntityFieldAudit
        {
